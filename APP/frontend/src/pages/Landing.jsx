...
import {
  CheckCircle, ShieldCheck, Robot, FileCsv, FilePdf, CalendarCheck,
  EnvelopeSimple, DeviceMobile, UsersThree, ArrowRight,
} from "@phosphor-icons/react";

const Feature = ({ Icon, title, desc }) => (
  <div className="border border-zinc-200 bg-white p-6">
    <Icon size={28} weight="bold" className="text-[#0033A0]" />
    <h3 className="font-display font-bold text-lg tracking-tight mt-3">{title}</h3>
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-zinc-200 bg-white">
        <div className="max-w-[1440px] mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#0033A0] flex items-center justify-center text-white font-display font-black tracking-tight">IL</div>
            <div>
          ...
          <nav className="flex items-center gap-2">
            <Link to="/login">
              <Button variant="outline" className="rounded-none border-zinc-300" data-testid="landing-signin-button">Sign in</Button>
            </Link>
            <Link to="/register">
              <Button className="rounded-none bg-[#0033A0] hover:bg-[#002266]" data-testid="landing-signup-button">Get started</Button>
            </Link>
          </nav>
        </div>
      </header>
      ...
      <section className="border-b border-zinc-200">
        <div className="max-w-[1440px] mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <div className="kbd-label">For Illinois Claimants & Case Workers</div>
            ...
            <p className="text-base text-zinc-700 mt-5 max-w-xl leading-relaxed">
              Track every work-search contact, certify Sun–Sat benefit weeks, and generate
              official-style ADJ034F reports for IDES — backed by a full audit trail.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/register">
                <Button className="rounded-none bg-[#0033A0] hover:bg-[#002266] h-12 px-6 font-semibold" data-testid="hero-cta-signup">
                  Create free account <ArrowRight size={16} weight="bold" className="ml-2" />
                </Button>
              </Link>
              <Link to="/login">
                <Button variant="outline" className="rounded-none border-zinc-300 h-12 px-6 font-semibold" data-testid="hero-cta-signin">
                  Have an invite? Sign in
                </Button>
              </Link>
            </div>
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-zinc-500">
              <span className="inline-flex items-center gap-1"><CheckCircle size={14} weight="fill" className="text-[#16A34A]" /> Free for claimants</span>
              <span className="inline-flex items-center gap-1"><CheckCircle size={14} weight="fill" className="text-[#16A34A]" /> No credit card</span>
              <span className="inline-flex items-center gap-1"><CheckCircle size={14} weight="fill" className="text-[#16A34A]" /> ADJ034F-style PDFs</span>
            </div>
          </div>
          <div className="relative border border-zinc-200 bg-white aspect-[4/3] overflow-hidden">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white border border-zinc-300 p-4">
              ...
              <div className="flex items-center justify-between mt-2">
                <div className="font-display font-bold text-xl tracking-tight">Apr 19 → Apr 25</div>
                <span className="inline-flex items-center gap-1 text-xs font-bold text-[#16A34A]">
                  <CheckCircle size={14} weight="fill" /> COMPLIANT 4/3
                </span>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-zinc-200">
        <div className="max-w-[1440px] mx-auto px-6 py-16">
          ...
          <h2 className="font-display font-bold text-3xl tracking-tight mt-2">Everything you need to stay certified</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
            <Feature Icon={CalendarCheck} title="Sun–Sat Benefit Weeks" desc="Auto-snap to Illinois' Sunday–Saturday week. ≥3 contacts enforced." />
            ...
            <Feature Icon={CheckCircle} title="Full audit trail" desc="Every edit captured with field-level old → new diffs." />
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="max-w-[1440px] mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <h2 className="font-display font-bold text-3xl tracking-tight mt-2">Invite a claimant in one click</h2>
            <p className="text-base text-zinc-700 mt-4 leading-relaxed">
              Send a single-use signup link to anyone you're helping. They land on a pre-filled
              registration page with their claimant profile already created.
            </p>
            <Link to="/login" className="inline-flex items-center gap-2 text-[#0033A0] font-semibold underline mt-4 text-sm">
              Admin sign-in <ArrowRight size={14} weight="bold" />
            </Link>
          </div>
      ...
      <footer className="border-t border-zinc-200 bg-[#F4F4F5]">
        <div className="max-w-[1440px] mx-auto px-6 py-6 flex items-center justify-between flex-wrap gap-3">
          <div className="text-xs text-zinc-500">
            Unofficial tool — not affiliated with the Illinois Department of Employment Security.
            Mirrors ADJ034F form structure for personal record-keeping.
          </div>
          <div className="flex gap-4 text-xs">
            <Link to="/login" className="text-[#0033A0] font-semibold underline">Sign in</Link>
            <Link to="/forgot-password" className="text-zinc-600 underline">Forgot password</Link>
          </div>
        </div>
      </footer>
    </div>
