    ...
    <div className="space-y-6" data-testid="calendar-page">
      <div>
        ...
        <p className="text-sm text-zinc-600 mt-2 max-w-2xl">
          Sunday rows highlight weeks you've logged. Green = compliant, red =
          under 3 contacts.
        </p>
      </div>
      ...
      <div className="border border-zinc-200 bg-white">
        ...
        <div className="grid grid-cols-7 border-b border-zinc-200">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div
              key={d}
              className="kbd-label py-2 px-3 text-center border-r last:border-r-0 border-zinc-200"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7">
          {cells.map((d, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`text-xs font-mono-data ${inMonth ? "text-zinc-900" : "text-zinc-400"}`}
                >
                  {d.getDate()}
                </div>
                {matched && (
                  ...
                  >
                    <div
                      className={`text-[10px] font-semibold uppercase tracking-wider inline-flex items-center gap-1 px-1.5 py-0.5 border ${matched.contact_count >= 3 ? "border-[#16A34A] text-[#16A34A] bg-green-50" : "border-[#DC2626] text-[#DC2626] bg-red-50"}`}
                    >
        ...
        <div className="px-6 py-3 border-t border-zinc-200 flex items-center gap-4 text-xs text-zinc-600">
          ...
          <span>
            {weeks.length} logged week{weeks.length === 1 ? "" : "s"} for active
            claimant
          </span>
        </div>
      </div>
