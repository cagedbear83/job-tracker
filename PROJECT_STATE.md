...
- [ ] **Not yet done:** files written directly into the local working tree, not yet committed/pushed
...
- [ ] **Not yet done:** none of this has been pushed/deployed yet — files were written directly into the local working tree (not yet committed). ⚠ Also touches `.env.example` with 6 new optional session-security vars — see Environment Variables Checklist below for prod-specific notes (cross-site cookie between Vercel frontend and DigitalOcean backend needs `REFRESH_COOKIE_SECURE=true` + `REFRESH_COOKIE_SAMESITE=none`, which are already the defaults)
